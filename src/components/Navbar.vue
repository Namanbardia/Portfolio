    ...
    <header class="flex justify-between items-center p-6 bg-opacity-50 relative z-20">
        ...
        <nav :class="[
            ...
            <ul class="flex flex-col items-center space-y-5 md:flex-row md:space-x-5 md:space-y-0">
                <li v-for="item in Menu" :key="item.name" class="group-relative" @click="scrollToSection(item.href)">
            ...
            <div class="mt-8 md:mt-0 md:ml-8">
                <button class="px-6 py-2.5 bg-linear-to-r from-blue-500 to-cyan-500 text-white rounded-full hover:shadow-lg hover:shadow-blue500/25 transition-all duration-300 hover:scale-105">
                    Let's Talk
                </button>
            </div>
        </nav>
